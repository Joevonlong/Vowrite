...
final class WhisperService {
    func transcribe(audioURL: URL, apiKey: String) async throws -> String {
        ...
        guard let httpResponse = response as? HTTPURLResponse else {
            throw VoxaError.networkError("Invalid response")
        }

        guard httpResponse.statusCode == 200 else {
            let errorBody = String(data: data, encoding: .utf8) ?? "Unknown error"
            throw VoxaError.apiError("STT API error (\(httpResponse.statusCode)): \(errorBody)")
        }

        guard let text = String(data: data, encoding: .utf8) else {
            throw VoxaError.apiError("Failed to decode STT response")
        }
