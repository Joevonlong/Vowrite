...
| ⎋ | **ESC 取消** | 随时按 ESC 取消录音 |
| 📊 | **历史记录** | 浏览和搜索过往的语音输入 |
| 🔌 | **多供应商** | OpenAI、Groq、DeepSeek、Ollama 等 |
| 🔑 | **密钥库** | API Key 按供应商存入 macOS 钥匙串，填一次全局复用 |
| ⚡ | **预设方案** | 一键配置（⭐ Groq 转录 + DeepSeek 润色、OpenAI 全家桶、本地 Ollama） |
| 🎨 | **个性化** | 快速偏好预设（商务、休闲、学术、创意、技术） |
...
| 供应商 | 语音模型 | 润色模型 | 备注 |
|--------|---------|---------|------|
| **⭐ Groq + DeepSeek** | whisper-large-v3-turbo | deepseek-chat | 推荐组合 |
| **OpenAI** | whisper-1 | gpt-4o-mini | 全家桶 |
| **Ollama** | — | 本地模型 | 100% 离线，本地运行 |
| OpenRouter | whisper-large-v3 | gpt-4o-mini | 多模型 |
| Together AI | whisper-large-v3 | Llama-3.1-8B-Instruct-Turbo | 开源模型 |
| 自定义 | 可配置 | 可配置 | 任何 OpenAI 兼容端点 |
