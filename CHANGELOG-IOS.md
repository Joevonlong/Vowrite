...
- **System dictation microphone hidden (F-076)**: the bottom-right system dictation mic + the duplicate system globe/dictation strip are now suppressed by three permanent mechanisms — (1) our own globe key is now **always rendered** (no longer gated behind `needsInputModeSwitchKey`); its `handleInputModeList(from:with:)` wiring tells iOS this extension owns input-mode switching, so iOS hides its own bottom globe+dictation strip (this is the actual mic-killer, the proven mechanism from the original "fix bottom duplicate bar" work that a later regression had gated off); (2) the keyboard declares itself multilingual + ASCII-capable (`PrimaryLanguage = mul`, `IsASCIICapable = YES`); (3) the keyboard backdrop is an opaque, appearance-adaptive color (UIKit + SwiftUI both opaque to the screen bottom) so the keyboard is one uniform color edge-to-edge with no two-tone seam, intentionally superseding the earlier translucent backdrop. A build-time guard (`ops/scripts/test.sh`) fails the suite if the plist declaration or the opaque backdrop is reverted.
