...
- **No more crash on a malformed custom polish endpoint**: If your custom Polish base URL was malformed (for example a pasted address with a stray space inside the host), the app could crash while building the request. It now surfaces a clear "Invalid polish endpoint URL" error instead, so a typo in settings never takes the app down.
