...
- **F-080 Personalization is now visible and controllable**: Settings → Personalization gains a Learning section showing what Vowrite has learned from your corrections — learned-rule and vocabulary counts plus the three most recent learned corrections (e.g. "语音识别 → Vowrite"). A master toggle turns auto-learning off entirely, and "Clear Learned Data" deletes everything learned (your manually created rules are never touched). Learned corrections are now stored as full replacement rules, so they're applied to future dictations, not just used as recognition hints.
