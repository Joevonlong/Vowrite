...
if [[ -x "$GUARD" && -n "$MAIN_ROOT" ]]; then
    ...
    feature_validator_chain_payload="$(jq -cn --arg cwd "$FEATURE_ROOT" --arg command 'ops/scripts/test.sh ; touch VowriteKit/outside.swift' '{cwd:$cwd,tool_name:"Bash",tool_input:{command:$command}}')"
