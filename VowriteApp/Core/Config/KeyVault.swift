...
enum KeyVault {
    ...
    static let managedProviders: [APIProvider] = [
        .groq,
        .deepseek,
        .openai,
        .openrouter,
    ...
    @discardableResult
    static func saveKey(_ key: String, for provider: APIProvider) -> Bool {
    ...
    @discardableResult
    static func deleteKey(for provider: APIProvider) -> Bool {
        guard provider.requiresAPIKey else { return true }
    ...
    static func maskedKey(for provider: APIProvider) -> String? {
        guard let key = key(for: provider) else { return nil }
        guard key.count > 8 else { return "••••••••" }
        return "\(key.prefix(4))••••\(key.suffix(4))"
    }

    static func requiredProviders(for configuration: SplitAPIConfiguration) -> [APIProvider] {
    ...
    static func missingProviders(for configuration: SplitAPIConfiguration) -> [APIProvider] {
