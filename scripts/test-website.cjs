...
const pages = ['index','why','apps','translate','pricing'];
...
(async () => {
  ...
  const failures=results.filter(x=>x.overflow||x.h1>1||x.duplicateIds?.length||x.brokenImages?.length);
  ...
  await test('language menu, mobile navigation, persistence and Escape focus',async()=>{
    ...
    await page.locator('.nav-menu-toggle').click();await page.locator('.nav-links a[href="apps.html"]').click();
    assert.equal(await page.locator('html').getAttribute('lang'),'de');
  ...
  await test('rich translations retain shortcut markup and real links in every language',async()=>{
    ...
    for(const l of ['en','zh','de','en']) {
      ...
      assert.equal(await page.locator('[data-i18n="ap.mac.flow.note"] a').getAttribute('href'),'translate.html#trig');
      assert.equal(await page.locator('[data-i18n="ap.ios.flow.note"] a').getAttribute('href'),'translate.html#trig');
    }
  });
  await test('all four original translation samples, pause, reduced motion',async()=>{
    await page.goto(`${base}/translate.html#trig`);
  ...
  await test('dictation, refinement, app styles, translation and provider configuration demos',async()=>{
    ...
    await page.locator('#vc-translate select').selectOption('Deutsch');assert.match(await page.locator('#vc-translate .vc-result').textContent(),/Donnerstag/);
  ...
  await test('old homepage anchors open their containing detail panels',async()=>{
    for(const id of ['how-it-works','compare','download-mac','translation-triggers']){
  ...
  await test('no JavaScript: complete content and navigation remain usable',async()=>{
    ...
    for(const route of pages){await p.goto(`${base}/${route}.html`);assert(await p.locator('.nav-links a[href="apps.html"]').isVisible());assert(await p.locator('h1').isVisible());}
  ...
  await test('blocked storage and failed i18n resource keep navigation and content',async()=>{
    ...
    const p=await c.newPage();await p.goto(`${base}/apps.html`);await p.locator('.lang-toggle').click();await p.locator('[data-lang="zh"]').click();assert.equal(await p.locator('html').getAttribute('lang'),'zh-CN');
    await p.route('**/i18n.js',r=>r.abort());await p.goto(`${base}/index.html`);assert(await p.locator('.nav-links a[href="apps.html"]').isVisible());assert(await p.locator('h1').isVisible());await c.close();
  });
  await test('WebKit: five routes, mobile dark layout and language switching',async()=>{
    ...
    for(const route of pages){await p.goto(`${base}/${route}.html`);await p.evaluate(()=>{setLang('de');document.querySelectorAll('details').forEach(x=>x.open=true)});assert(await p.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));}
